...
Let's add country code to the mobile data so that we can merge the GNI data by a common value later. We'll exlclude plans that include voice and text and keep only the relevant features.
```{r}
...
# getting data only plans
mobile <- subset(mobile, Voice...SMS.Included. == "No")
...
#combine$cost.local <- signif(combine$cost.local, digits = 3)
...
final_df <- combine %>%
  ...
  summarise(total.amount = sum(data.amount),
            ...
            total.plans = n()) %>%
  mutate(cost.usd.per.gb = total.cost/total.amount/total.plans)
```
...
final_df <- final_df %>%
  ...
  select(-total.amount, -total.cost, -total.plans)
```
...
check <- subset(final_df, cost.usd.per.gb > 40)
```



Looking at some of the costs in the original mobile data it seems there may have been some bad data entry so I'm going to exclude anything above $30/GB...
```{r}
final_df <- subset(final_df, cost.usd.per.gb <= 30)
summary(final_df)
ggplot(aes(x = cost.usd.per.gb), data = final_df) +
  geom_histogram()
```
