...
```{r mobile_import}
mobile <- read.csv("mobile-parameters-q2-2015.csv",
...
Let's get cost per gb for each plan...
```{r}
combine <- combine %>% mutate(cost.gb.usd = cost.usd / data.amount)
```


Only keep data plans...?
```{r}

```


Now let's group by country and get an average cost per GB by adding all the cost per GB for a country and then dividing by the number of plans...
...
final_df <- combine %>%
  ...
  summarise(count = n(), total.cost.gb.usd = sum(cost.gb.usd)) %>%
...
final_df <- final_df %>%
  ...
  select(-count, -total.cost.gb.usd)
```
...
#final renames to sync up with geojson file
# Dem. Rep. of Congo
index <- final_df$country.code == "CRI"
final_df$country.code[index] <- "COD"
# Romania
index <- final_df$country.code == "PRT"
final_df$country.code[index] <- "ROU"
# West Bank/Palestine
index <- final_df$country.code == "NPL"
final_df$country.code[index] <- "PSE"
